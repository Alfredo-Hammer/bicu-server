...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE users ADD COLUMN movil VARCHAR(20);
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'users' AND column_name = 'profesion'
    ) THEN
        ALTER TABLE users ADD COLUMN profesion VARCHAR(100);
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'users' AND column_name = 'estado'
    ) THEN
        ALTER TABLE users ADD COLUMN estado VARCHAR(50);
